...
- `rpx.lock` records the exact R version, repositories, package versions, sources, and system requirements selected by the resolver.
...
The project itself must be an installable R package. Its `DESCRIPTION` needs valid `Package` and `Version` fields, and the project must contain the files required by `R CMD INSTALL`.

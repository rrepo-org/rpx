...
`add` and `remove` write `DESCRIPTION` and `rpx.lock` before synchronizing packages. If a later build or installation fails, the metadata remains updated and the library can be partially synchronized. rpx cleans temporary build/install workspaces and lock directories, so fix the reported problem and retry with `rpx sync`; an `rpx clean` should not normally be required.
